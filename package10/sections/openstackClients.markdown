...
In a pervious version of OpenStack, command line clients were split into several tools, called **nova, glance, cinder, swift, keystone** and more. Each of the tools was dedicated to certain tasks, e.g. *nova* for compute services, *glance* for managing Images, etc. Refer to the [official OpenStack clients website][OpenStackClients] for more details.
